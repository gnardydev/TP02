...
void OrdenarVetorParcialInsertionNome(int tam, int k);
...
int main(){
    ...
    int k = 10;
    OrdenarVetorParcialInsertionNome(cont, k);
    ...
    t = clock() - t;
    FILE *arq = fopen("matricula_insertionsort.txt", "w");
...
//funçao para ordenação parcial com insertionsort
void OrdenarVetorParcialInsertionNome(int tam, int k){
    for(int i = 1; i < tam; i++){
        Jogador tmp = jogadores2[i];
        int j = (i<k) ? i-1 : k-1;
        comparacoes+=2;
        while((j >= 0) && (jogadores2[j].ano > tmp.ano)){
            movimentacoes++;
